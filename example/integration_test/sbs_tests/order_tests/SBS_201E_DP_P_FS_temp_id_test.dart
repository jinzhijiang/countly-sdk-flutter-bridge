...
/// - Key length 6, value size 4 and breadcrumb count 99 in P
/// - The event is recorded with the key truncated to 6 #P because of temporary id, sbs fetch did not sent
...
void main() {
  ...
  testWidgets('SBS_201E_DP_P_FS_test', (WidgetTester tester) async {
    ...
    // Initialize the SDK
    CountlyConfig config = CountlyConfig(TEST_SERVER_URL, APP_KEY).enableManualSessionHandling().setLoggingEnabled(true).enableTemporaryDeviceIDMode();
    ...
    validateEvent(event: jsonDecode(eq.first), key: 'ThisWi', segmentation: {'no1': 'value', 'no4': 'value', 'no3': 'value', 'no5': 'value'});

    expect(await getServerConfig(), {
      'v': 1,
      't': 1750748806691,
      'c': {'lkl': 6, 'lsv': 4, 'lbc': 99}
    });
  });
}
